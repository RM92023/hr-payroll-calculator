...
export abstract class PayrollCalculatorTemplate {
  ...
  calculate(input: PayrollInput): PayrollResult {
    ...
    const { taxes, taxDetail, mandatoryDeductions } = this.computeDeductions(gross, input);
    const otherDeductions = this.normalizeMoney(input.otherDeductions ?? 0);

    if (otherDeductions > gross) throw new Error('otherDeductions cannot exceed gross');

    const net = this.round2(gross - taxes - mandatoryDeductions - otherDeductions);
  ...
  protected validate(input: PayrollInput) {
    ...
    if ((input.otherDeductions ?? 0) < 0) throw new Error('otherDeductions must be >= 0');
  }

  protected computeGross(input: PayrollInput) {
    return this.normalizeMoney(input.baseSalary) + this.normalizeMoney(input.bonuses ?? 0);
  }

  protected computeDeductions(gross: number, input: PayrollInput) {
    const strategy = this.taxStrategies.find(s => s.supports(input));
    if (!strategy) throw new Error(`No tax strategy for contractType=${input.contractType}`);

    const { total: taxes, detail: taxDetail } = strategy.computeTaxes(gross, input);

    // Ejemplo: deducción obligatoria adicional ficticia para empleados
    const mandatoryDeductions = input.contractType === 'EMPLOYEE' ? this.round2(gross * 0.01) : 0;

    return { taxes: this.round2(taxes), taxDetail, mandatoryDeductions };
  }

  protected normalizeMoney(n: number) { return this.round2(n); }
  protected round2(n: number) { return Math.round(n * 100) / 100; }
}
