...
export interface TaxStrategy {
  ...
  computeTaxes(gross: number, input: PayrollInput): { total: number; detail: Record<string, number> };
}

export class EmployeeTaxStrategy implements TaxStrategy {
  supports(input: PayrollInput) { return input.contractType === 'EMPLOYEE'; }
  ...
  computeTaxes(gross: number) {
    ...
    const withholding = Math.max(0, (gross - 2_000_000) * 0.10);
...
export class ContractorTaxStrategy implements TaxStrategy {
  supports(input: PayrollInput) { return input.contractType === 'CONTRACTOR'; }
